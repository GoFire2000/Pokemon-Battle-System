...
int getRand(int x, int y) {//从x到y之间的随机数

	if (x > y) swap(x, y);
	return x + rand() % (y - x + 1);
}

int getExperience(int Level) {//得到升级到Level的经验值
	int exp = 0;
...
bool Power::attack(string& verStr, Pokemon & attacker, Pokemon & target, int skillId) {
	...
	verStr += "精灵" + attacker._name + "对精灵" + target._name + "使用技能" + attacker.getSkillName(skillId) + ",";
	if (skillId == 0) {//撞击
		...
		verStr += "提升了自身" + to_string(attacker._nowAtk / 10) + "点攻击力。\n"; verStr += "#";
		return false;
	}
...
bool MeatShield::attack(string& verStr, Pokemon & attacker, Pokemon & target, int skillId) {
	...
	if (skillId == 0) {//撞击
		...
		cout << "提升了自身" << attacker._nowHp / 6 << "点生命值。\n" << endl;
		verStr += "提升了自身" + to_string(attacker._nowHp / 6) + "点生命值。\n"; verStr += "#";
		return false;
	}
...
bool Defensive::attack(string& verStr, Pokemon & attacker, Pokemon & target, int skillId) {
	...
	if (skillId == 0) {//撞击
		...
		cout << "提升了自身" << attacker._nowDef / 10 << "点防御力。\n" << endl;
		verStr += "提升了自身" + to_string(attacker._nowDef / 10) + "点防御力。\n"; verStr += "#";
		return false;
	}
...
bool Agile::attack(string& verStr, Pokemon & attacker, Pokemon & target, int skillId) {
	...
	if (skillId == 0) {//撞击
		...
		cout << "提升了自身" << attacker._nowSp / 10 << "点速度。\n" << endl; 
		verStr += "提升了自身" + to_string(attacker._nowSp / 10) + "点速度。\n"; verStr += "#";
...
bool Pokemon::gainExp(string& verStr, int val) {
	...
	verStr += "精灵" + _name + "获得" + to_string(val) + "经验值!";
	verStr += "#";
	...
	if (tag) {
		...
		verStr += "恭喜，精灵" + _name + "升级成功，当前等级为" + to_string(_level) + "!";
		verStr += "##";
		return true;
	}
	cout << endl;
